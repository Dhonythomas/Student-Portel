document.addEventListener('DOMContentLoaded', () => {
    ...
    // This object maps the department name to its specific course
    // (BSc for sciences, BA for arts/humanities)
...
if (courseBtn) {
    courseBtn.addEventListener('click', () => {
        courseBtn.disabled = true; // optional: prevents double-clicks
        courseBtn.innerHTML = `<span class="spinner"></span> Processing...`; // optional spinner text

        setTimeout(() => {
            submitCourseRequest();
            courseBtn.disabled = false;
            courseBtn.innerHTML = "Submit"; // restore text after delay
        }, 1000); // 1000ms = 1 second delay
    });
}


    // --- EXAM REGISTER SECTION ---
    ...
    if (registerExamBtn) {
        registerExamBtn.addEventListener('click', async () => {
            ...
            const studentId = document.getElementById('examRollNumber').value.trim();
            ...
            courseEntries.forEach((entry, index) => {
                const code = entry.querySelectorAll('input')[0].value.trim();
                if (code) courseCodes.push({ [`course${index + 1}`]: code });
            });

            const examDetails = {
                studentName, studentId, examName, examCode, department,
                semester, amount, courses: courseCodes
            };

            if (examConfirmBtn) {
                examConfirmBtn.dataset.examDetails = JSON.stringify(examDetails);
            }
            showExamPaymentModal(examDetails);
        });
    }
    ...
    // ================================================================================= //
    // ALL SUBSEQUENT FUNCTIONS ARE NOW MOVED INSIDE DOMContentLoaded TO FIX SCOPE ISSUES //
    // ================================================================================= //
    ...
    try {
        ...
        feeData.forEach(fee => {
            const formattedAmount = parseFloat(fee.amount).toLocaleString('en-IN', {
                style: 'currency',
                currency: 'INR'
            });
            const formattedDate = fee.lastdate
                ? new Date(fee.lastdate).toLocaleDateString('en-IN', {
                      day: '2-digit',
                      month: 'short',
                      year: 'numeric'
                  })
            ...
            const fileUrl = fee.fee_structure_path ? fee.fee_structure_path.trim() : '';

            const feeItem = document.createElement('div');
            feeItem.className = 'notification-item fee-item';
            feeItem.innerHTML = `
                <div class="fee-item-details">
                    <p class="notification-title">${fee.feeTitle}</p>
                ...
                <div class="fee-item-action">
                    ...
                    ${
                        fileUrl
                            ? `<button class="view-structure-btn" data-file="${fileUrl}">View Structure</button>`
                            : ''
                    }
                </div>`;
        ...
        feeListContainer.addEventListener('click', event => {
            if (event.target.classList.contains('pay-fee-btn')) {
                const button = event.target;
                const feeDetails = {
                ...
                showFeeDetailsModal(feeDetails);
            }

            if (event.target.classList.contains('view-structure-btn')) {
                const fileUrl = event.target.dataset.file;
                if (fileUrl) {
                    window.open(fileUrl, '_blank'); // open PDF/image in new tab
                } else {
                    alert('Fee structure file not available.');
                }
            }
        });
    } catch (error) {
    ...
    function updateUI(currentStageData) {
        ...
        if (currentStatus === 'Rejected') {
            ...
            for (let i = 0; i < rejectedStageIndex; i++) {
                if (stageElements[stageOrder[i]]) stageElements[stageOrder[i]].classList.add('completed');
            }
            if (stageElements[currentStage]) stageElements[currentStage].classList.add('rejected');
        } else {
            ...
            if (currentIndex < stageOrder.length - 1) {
                ...
                if (stageElements[nextStage]) stageElements[nextStage].classList.add('active');
            }
        }
    ...
    async function profileload() {
        try {
            ...
            if (data.status === 'success') {
                ...
                document.getElementById('popupPic').src = studentData.profile_pic;
                document.getElementById('profilePicheader').src = studentData.profile_pic;
                ...
                document.getElementById("joinedDate").value = studentData.joiningdate;
                ...
                document.getElementById("parentPhone").value = studentData.ParentPho;
                document.getElementById("address").value = studentData.address;
                document.getElementById("ProfilePic").src = studentData.profile_pic;
                loadFeeNotifications();
    ...
    async function submitCourseRequest() {
        ...
        async function prepareFileData(file) {
            ...
            return { fileName: file.name, fileType: file.type, FileData: base64 };
        }
        ...
        try {
            ...
            const result = await response.json();
            if (result.success) {
                showPopup("✅ Course certificate request submitted successfully!");
                ...
                showPopup("⚠️ Error: " + (result.message || "Could not submit request."));
            }
        } catch (error) {
    ...
    // THIS FUNCTION IS NOW GLOBAL SO HTML ONCLICK CAN ACCESS IT
    ...
    function showExamPaymentModal(details) {
        ...
        document.getElementById('examPayAmount').textContent = parseFloat(details.amount).toLocaleString('en-IN', { style: 'currency', currency: 'INR' });
    ...
    async function processExamPayment(event) {
        ...
        examDetails.upiId = upiId;

        try {
            const response = await fetch('conformexampayment.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    // CORRECTED AND COMPLETED THE INCOMPLETE FUNCTION
    async function addCourseEntry() {
        const mailid = document.getElementById('email').value.trim();
        // Add the rest of the logic for this function here
    }
